...
首先读取 `~/.ralph/config.json` 获取 `activeProject` 名称，然后使用 Read 工具读取 `~/.ralph/projects/<activeProject>/.ralph-pid` 文件。
...
- 如果输出 "dead"：通过 Bash 工具执行 `rm ~/.ralph/projects/<activeProject>/.ralph-pid` 清理过期 PID 文件，然后报告"Ralph 当前未在运行（已清理过期 PID 文件）。"并**停止**
- 如果输出 "alive"：继续
...
```bash
rm -f ~/.ralph/projects/<activeProject>/.ralph-pid
```
